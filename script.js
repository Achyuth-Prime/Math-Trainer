...
function updateStats(level, newTime, wrongCount = null) {
  ...
  const newAvg =
    totalPlays === 0 ? newTime : (oldAvg * totalPlays + newTime) / newPlays;
  ...
  appDiv.innerHTML = `
        <div class="glass-panel">
            ...
            <div class="level-grid" style="grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));">
                ...
                <div class="level-card" style="grid-column: 1 / -1;" onclick="renderPercentageHome()">
                    <h2>% Percentage Trainer</h2>
                </div>
            </div>
        </div>
    `;
...
function renderGame() {
  ...
  if (!gameUI) {
    ...
    document.getElementById("add-header").innerText =
      `Level ${currentLevel} - Round ${currentStartPointIndex + 1} / ${startPoints.length}`;
...
function renderSquaresGame() {
  ...
  const modeNames = {
    rapid: "Rapid",
    absolute: "Absolute",
    sq_rand: "Squares Random",
    sq_ord: "Squares Ordered",
    cb_rand: "Cubes Random",
    cb_ord: "Cubes Ordered",
  };
  ...
  if (!gameUI) {
    ...
    if (!sqIsReviewing) {
      document.getElementById("sq-header").innerText =
        `${modeTitle} Mode - ${sqQuestions.length + 1} remaining`;
...
function renderSquaresSummary() {
  ...
  const { oldAvg, newAvg } = updateStats(
    statKey,
    avgTimePerProblem,
    sqWrongCount,
  );
...
function renderFactorialsGame() {
  ...
  if (!gameUI) {
    ...
    if (!fcIsReviewing) {
      document.getElementById("fc-header").innerText =
        `${modeTitle} Mode - ${fcQuestions.length + 1} remaining`;
...
function renderTablesGame() {
  ...
  if (!gameUI) {
    ...
    if (!tbIsReviewing) {
      document.getElementById("tb-header").innerText =
        `${modeTitle} - ${tbQuestions.length + 1} remaining`;
...
function renderSquareRootsGame() {
  ...
  const modeTitle =
    srMode === "ord" ? "Square Roots Scheduled" : "Square Roots Random";
  ...
  if (!gameUI) {
    ...
    if (!srIsReviewing) {
      document.getElementById("sr-header").innerText =
        `${modeTitle} - ${srQuestions.length + 1} remaining`;
...
function getPowersData(type) {
  ...
  if (type === "2") {
    for (let i = 4; i <= 10; i++)
      questions.push({ q: `2<sup>${i}</sup>`, a: Math.pow(2, i) });
  } else if (type === "3") {
    for (let i = 4; i <= 8; i++)
      questions.push({ q: `3<sup>${i}</sup>`, a: Math.pow(3, i) });
  } else if (type === "4") {
    for (let i = 4; i <= 6; i++)
      questions.push({ q: `4<sup>${i}</sup>`, a: Math.pow(4, i) });
  } else if (type === "5") {
    for (let i = 4; i <= 6; i++)
      questions.push({ q: `5<sup>${i}</sup>`, a: Math.pow(5, i) });
  } else if (type === "others") {
    for (let i = 6; i <= 9; i++)
      questions.push({ q: `${i}<sup>4</sup>`, a: Math.pow(i, 4) });
  }
  return questions;
}
...
function renderPowersGame() {
  ...
  if (!gameUI) {
    ...
    if (!pwIsReviewing) {
      document.getElementById("pw-header").innerText =
        `${modeTitle} - ${pwQuestions.length + 1} remaining`;
...
let pcWrongCount = 0;
let pcCurrentQ = null;
let pcQuestionStartTime = 0;
let pcIsReviewing = false;
...
function renderPercentageHome() {
    appMode = 'percentage';
    const stats = getStats();
    
    function createStatCard(modeId, title, desc) {
        const s = stats[`pc_${modeId}`] || { plays: 0, avgTime: null, lastWrong: 0 };
        const text = s.plays > 0 
            ? `<p style="color: var(--primary); font-weight: bold; margin-top: 0.5rem; font-size: 1rem;">Played: ${s.plays} | Avg: ${s.avgTime.toFixed(2)}s <br> PIA: ${s.lastWrong || 0}</p>`
            : `<p style="color: grey; font-size: 1rem; margin-top: 0.5rem;">Not played yet</p>`;
        return `
            <div class="level-card" onclick="startPercentageLevel('${modeId}')">
                <h2>${title}</h2>
                <p>${desc}</p>
                ${text}
            </div>
    ...
    appDiv.innerHTML = `
        <div class="glass-panel">
            <h1> Percentage Trainer </h1>
            <div class="level-grid" style="grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));">
                ${createStatCard('rand_10', 'Random 10', '10 random questions')}
                ${createStatCard('rand_20', 'Random 20', '20 random questions')}
                ${createStatCard('ord', 'Percentage Scheduled', 'In Order')}
                ${createStatCard('rand', 'Percentage Random', 'Shuffled Order')}
            </div>
            <div class="actions-row" style="margin-top: 3rem;">
                <button class="action-btn secondary" onclick="renderMainMenu()">Main Menu</button>
            </div>
...
function startPercentageLevel(mode) {
    pcMode = mode;
    pcWrongCount = 0;
    pcResults = [];
    pcIsReviewing = false;
    ...
    if (mode === 'rand_10') {
        pcQuestions = shuffle([...data]).slice(0, 10);
    } else if (mode === 'rand_20') {
        pcQuestions = shuffle([...data]).slice(0, 20);
    } else if (mode === 'rand') {
        pcQuestions = shuffle([...data]);
    } else {
        pcQuestions = [...data];
    }
    
    renderPercentageGame();
}

function renderPercentageGame() {
    if (pcQuestions.length === 0) {
        renderPercentageSummary();
        return;
    }
    
    if (!pcIsReviewing) {
        pcCurrentQ = pcQuestions.shift();
    }
    
    let modeTitle = 'Percentage Trainer';
    if (pcMode === 'rand_10') modeTitle = 'Random 10';
    else if (pcMode === 'rand_20') modeTitle = 'Random 20';
    else if (pcMode === 'ord') modeTitle = 'Percentage Scheduled';
    else if (pcMode === 'rand') modeTitle = 'Percentage Random';

    let gameUI = document.getElementById('pc-game-ui');

    if (!gameUI) {
        appDiv.innerHTML = `
            <div id="pc-game-ui" class="glass-panel game-container" style="animation: slideUp 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards;">
                <h2 id="pc-header">${modeTitle} - ${pcQuestions.length + 1} remaining</h2>
                
                <div id="pc-question" style="font-size: 6rem; font-weight: bold; margin: 3rem 0; color: var(--text-main); text-shadow: 0 0 20px var(--primary-glow);">${pcCurrentQ.q} = ?%</div>
                
                <div class="input-section">
                    <p id="pc-feedback-text" style="font-size: 1.5rem; color: var(--text-muted); margin-bottom: 2rem; display: none;">Memorize it. We'll ask you this again later.</p>
                    <form id="pc-answer-form">
                        <input type="number" step="any" id="pc-input" class="sum-input" autocomplete="off" autofocus required placeholder="Answer (%)">
                        <br>
                        <button type="submit" class="submit-btn" id="pc-submit-btn">Submit Answer</button>
                    </form>
                </div>
            </div>
        `;
        
        const form = document.getElementById('pc-answer-form');
        const input = document.getElementById('pc-input');
        
        setTimeout(() => {
            input.focus();
            pcQuestionStartTime = performance.now();
        }, 100);
        ...
        form.onsubmit = (e) => {
            ...
            const btnEl = document.getElementById('pc-submit-btn');
            const fbEl = document.getElementById('pc-feedback-text');
            const containerEl = document.getElementById('pc-game-ui');
            ...
            const endTime = performance.now();
            const t = ((endTime - pcQuestionStartTime) / 1000);
            const answer = inputEl.value.trim(); // Handle as string match
            
            if (answer === pcCurrentQ.a) {
                pcResults.push(t);
                inputEl.value = '';
                pcQuestionStartTime = performance.now();
                renderPercentageGame(); 
                ...
                pcWrongCount++;
                pcQuestions.push(pcCurrentQ); // Re-queue at the end
                
                pcIsReviewing = true;
                containerEl.style.borderColor = 'var(--error)';
                containerEl.style.boxShadow = '0 0 30px rgba(255,0,0,0.3)';
                
                qEl.innerHTML = `${pcCurrentQ.q} = <span style="color: var(--success); text-shadow: 0 0 20px var(--success);">${pcCurrentQ.a}%</span>`;
        ...
        if (!pcIsReviewing) {
            document.getElementById('pc-header').innerText = `${modeTitle} - ${pcQuestions.length + 1} remaining`;
            document.getElementById('pc-question').innerText = `${pcCurrentQ.q} = ?%`;
        }
    }
}

function renderPercentageSummary() {
    const totalTime = pcResults.reduce((a, b) => a + b, 0);
    const statKey = `pc_${pcMode}`;
    const { oldAvg, newAvg } = updateStats(statKey, totalTime, pcWrongCount);
    ...
    appDiv.innerHTML = `
        <div class="glass-panel">
            ...
            <div class="summary-details">
                <div class="detail-item">
                    <div class="detail-label">Questions Solved</div>
                    <div class="detail-value text-success">${pcResults.length}</div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">Mistakes Made</div>
                    <div class="detail-value text-error">${pcWrongCount}</div>
                </div>
            </div>
            ...
            <div class="actions-row">
                <button class="action-btn" onclick="startPercentageLevel('${pcMode}')">Retry</button>
                <button class="action-btn secondary" onclick="renderPercentageHome()">Percentage Menu</button>
            </div>
        </div>
    `;
}
